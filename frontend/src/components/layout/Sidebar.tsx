...
const Logo = ({ collapsed }: { collapsed: boolean }) => (
  <div className={`flex items-center h-14 border-b border-neutral-700 ${collapsed ? 'justify-center px-2' : 'justify-start px-6'}`}>
    {collapsed ? (
      <Image
        src="/logo.png"
        alt="Logo"
      ...
      <div className="flex items-center space-x-3">
        <Image
          ...
          className="rounded-lg"
        />
        <h1 className="text-xl font-semibold text-neutral-100 whitespace-nowrap">打工人必备工具</h1>
      </div>
    )}
  </div>
);
              ...
              <Link
                ...
                className={cn(
                  'flex items-center w-full p-2.5 rounded-md text-sm font-medium hover:bg-neutral-700 hover:text-neutral-100 transition-colors duration-150',
                  {
                    ...
                    'bg-sky-600 text-white': pathname === category.href,
                  }
                )}
              >
          ...
          return (
            <div key={category.label}>
              ...
              >
                <category.icon className={cn('h-5 w-5', isCollapsed ? 'mx-auto' : 'mr-3')} />
                {!isCollapsed && <span className="flex-1 text-left whitespace-nowrap">{category.label}</span>}
                {!isCollapsed && category.children.length > 0 && (openCategories.includes(category.label) ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />)}
              </button>
              ...
              {isCollapsed && openCategories.includes(category.label) && category.children.length > 0 && (
